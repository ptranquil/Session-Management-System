 ...
 */
const isSlotAvailable = async (startTime: any, endTime: any, therapistId: any) => {
...
export const addSession = async (req: Request, res: Response) => {
    try {
        ...
        const temp = await isSlotAvailable(startTime, endTime, req.body.therapistId);
        if (temp) {
...
export const getSession = async (req: Request, res: Response) => {
    try {
        // const sessions = await sessionModel.find({ isActive: true }).populate("patientId", "therapistId");
...
export const getSessionByID = async (req: Request, res: Response) => {
    try {
        ...
        const session = await sessionModel.findOne({ _id: sessionid, isActive: true });
        if (!session) {
            anonymousErrors(404, config.SESSIONS.NOTFOUND, res);
            return;
        }
        apiSuccess(200, config.SESSIONS.FETCH, res, session);
        return;
    } catch (error: any) {
